...
describe(some.name, () => {
  ...
  describe('with single validator', () => {
    ...
    const validator = some(itemValidator)
    ...
    it('fails if all elements are invalid', () => {
      const itemValidator: Validator<number> = (value) => (value !== 0 ? failure(value, '') : value)
      const validator = some(itemValidator)
  ...
  describe('with multiple validators', () => {
    ...
    const validator = some(itemValidator1, itemValidator2)
    ...
    it('aggregates error messages from all items', () => {
      ...
      expect(Object.keys(result.errors[0]?.details ?? {})).toHaveLength(2)
      expect(Object.keys(result.errors[1]?.details ?? {})).toHaveLength(2)
    })
    ...
    it('adds the item index to path for errors', () => {
      ...
      expect(result.errors[1]?.path).toEqual(['1'])
      expect(result.errors[2]?.path).toEqual(['2'])
    })
  })

  it('resolves "and" shorthand', () => {
    const validator: Validator<number> = (value) => value
    expect(some([validator, validator])([1])).toEqual([1])
  })

  it('resolves object shorthand', () => {
    const validator: Validator<number> = (value) => value
    const value = [{ n: 1 }]
    expect(some({ n: validator })(value)).toBe(value)
  })
})
