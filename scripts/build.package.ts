...
import { execAsync, ExecReturnValue } from './util.js'
...
async function build() {
  ...
  await executeStep(`Compiling`, () => execAsync(`tsc -p ${PACKAGE_DIR}/tsconfig.build.json`))
  ...
  if (fs.existsSync(configPath)) {
    await executeStep(`Bundling type definitions`, async () => {
      ...
      if (!extractorResult.succeeded) {
        shell.echo(
          `API Extractor completed with ${chalk.yellow(`${extractorResult.errorCount}`)} ${chalk.red(
            'errors',
          )} and ${chalk.yellow(`${extractorResult.warningCount}`)} ${chalk.cyan('warnings')}`,
        )
      ...
      shell.mv(
        ...
        path.join(OUTPUT_DIR, `${PROJECT_NAME}-${PACKAGE_SIMPLE_NAME}.d.ts`),
      )

      const indexDefContent = `export * from './${PROJECT_NAME}-${PACKAGE_SIMPLE_NAME}'\n`

      await promisify(fs.writeFile)(path.join(OUTPUT_DIR, 'index.d.ts'), indexDefContent)
  ...
  await executeStep(`Compiling esm2015`, async () => {
    ...
    await promisify(fs.writeFile)(path.join(ESM2015_DIR, 'package.json'), `{"type":"module"}\n`)
  ...
  const globalsArg = externalsArr.length > 0 ? `-g ${externalsArr.map((e) => `${e}:${GLOBALS[e] || e}`).join(',')}` : ''
  ...
  await executeStep(`Bundling UMD (DEV)`, async () => {
    const ret = await execAsync(
      ...
      `-o ${UMD_DIR}/${PROJECT_NAME}.${PACKAGE_SIMPLE_NAME}.development.js`,
      `-n ${PACKAGE.name}`,
      `-m`,
      `--exports named`,
      externalsArg,
  ...
  await executeStep(`Bundling CJS (DEV)`, async () => {
    const ret = await execAsync(
      ...
      `-f cjs`,
      `-i ${TEMP_DIR}/bundle.es5.js`,
      `-o ${CJS_DIR}/${PROJECT_NAME}.${PACKAGE_SIMPLE_NAME}.development.js`,
      `-n ${PACKAGE.name}`,
      `-m`,
      `--exports named`,
      externalsArg,
...
`,
  ...
  await executeStep(`Cleaning build files`, async () => {
    ...
    const srcFiles = await promisify(glob)(`${OUTPUT_DIR}/src/**/*`, { nodir: true })
  ...
  shell.echo(chalk.green(`Finished building package '${chalk.cyan(PACKAGE.name)}' in ${formattedBuildTime}!`))
}
